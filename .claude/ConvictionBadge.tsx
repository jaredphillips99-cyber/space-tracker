...
export function ConvictionBadge({ rating, rationale, size = 'md' }: Props) {
  ...
  return (
    <div className="flex flex-col gap-1">
      <span
        className={`inline-flex items-center gap-1.5 rounded font-mono font-semibold uppercase tracking-wider ${sizeStyles[size]}`}
        style={{
          color,
          background: `${color}18`,
      ...
      {rationale && (
        <p className="text-xs leading-snug" style={{ color: '#8b93a8' }}>
          {rationale}
        </p>
      )}
    </div>
